...
\caption{Performance across four recognition domains. CU achieves 3--4$\times$ RTE speedup with competitive forgetting, retention, and privacy.}
\label{tab:all}
\end{table*}
...
\begin{figure}[!ht]
    ...
    \caption{t-SNE embeddings: baselines collapse to one point; CU distributes to nearest boundaries.}
...
\end{tabular}
\vspace{0.2cm}
\caption{Impact of replay buffer size. A 0.1 ratio balances retention and efficiency.}
...
\end{tabular}
\vspace{0.2cm}
\caption{Retain dominance constraint ablation: eliminates long-tail convergence.}
...
\end{tabular}
\vspace{0.2cm}
\caption{Localized regularization lowers total $\ell_2$ norm with minimal accuracy loss.}
...
\begin{figure}[!ht]
    ...
    \caption{$\ell_2$ norm per block: updates concentrate in a few blocks.}
...
\end{tabular}
\vspace{0.2cm}
\caption{Effect of ViT backbone scale on unlearning.}
\label{tab:vit_scale}
\end{table}
...
\begin{figure}[!h]
    ...
    \vspace{0.1cm}
    \caption{Retain accuracy across sequential cycles: baselines degrade progressively, CU stays stable.}
...
\end{tabular}}
\vspace{0.15cm}
\caption{\textcolor{red}{FAR/EER of forget probes against the retain gallery (CASIA-Iris).}}
\label{tab:far_eer}
\end{table}
...
\begin{figure}[!ht]
    ...
    \caption{Attention maps on forget and retain samples (input, base, retrained, CU). \textcolor{red}{Retrained/CU models disperse attention off-face only for forget identities, confirming surgical removal.}}
...
\begin{figure}[!ht]
    ...
    \caption{CU recovers gradually (genuine forgetting) vs.\ rapid head-masking recovery (superficial).}
